...
//    LOGD("getSoundTouchData +");
    while (playStatus != NULL && !playStatus->exit)
    {
        ...
        if (num == 0)
        {//TODO 只有在receiveSamples返回num为0时，才算finished
            isFinished = true;
            continue;
        }
        else
...
void pcmBufferCallBack(SLAndroidSimpleBufferQueueItf bf, void *context)
{
    ...
    if (myAudio != NULL)
    {
        ...
        if (bufferSize > 0)
        {
            ...
            if (myAudio->isRecordPcm)
            {
                myAudio->callJava->onCallEncodePCM2AAC(CHILD_THREAD, bufferSize * 2 * 2, myAudio->sampleBuffer);
            }
...
void MyAudio::startStopRecord(bool startRecord)
{
    this->isRecordPcm = startRecord;
}
